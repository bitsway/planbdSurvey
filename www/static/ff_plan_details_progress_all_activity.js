...
var apipath1="http://127.0.0.1:8000/planbd/analysis/";
//---- online
...
function field_force_detail(){	
	...
	//alert(apipath1+'sync_ff_detail?cid=WAB&mobile='+localStorage.mobile_no+'&sync_code='+localStorage.sync_code);
		$.ajax({
				url:apipath1+'get_ff_detail?cid=PLANBD&mobile='+localStorage.mobile_no+'&sync_code='+localStorage.sync_code,
...
function ff_plan_all_activity_progress() {		
		...
		if (from_year=="" || from_month=="" || to_year=="" || to_month=="" ){
			...
			$.ajax({
				url:apipath1+'get_ff_plan_details_all_activity?cid=PLANBD&mobile='+localStorage.mobile_no+'&sync_code='+localStorage.sync_code+'&from_year='+from_year+'&from_month='+from_month+'&to_year='+to_year+'&to_month='+to_month+'&sector='+sector,
			  success: function(result) {
				plan_list=result;				
...
function ff_ach_all_activity_progress() {
		...
		//alert(apipath1+'sync_ff_achievement_details_all_activity?cid=WAB&mobile='+localStorage.mobile_no+'&sync_code='+localStorage.sync_code+'&from_year='+from_year+'&from_month='+from_month+'&to_year='+to_year+'&to_month='+to_month+'&sector='+sector);
			$.ajax({
				url:apipath1+'get_ff_achievement_details_all_activity?cid=PLANBD&mobile='+localStorage.mobile_no+'&sync_code='+localStorage.sync_code+'&from_year='+from_year+'&from_month='+from_month+'&to_year='+to_year+'&to_month='+to_month+'&sector='+sector,
			  success: function(result) {
				ach_list=result
					//alert(ach_list);
					compare_data()
